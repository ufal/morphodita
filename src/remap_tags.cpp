...
int main(int argc, char* argv[]) {
  ...
  while (getline(stdin, line)) {
    ...
    // Analyse
    d->analyze(tokens[0].c_str(), tokens[0].size(), morpho::NO_GUESSER, lemmas);
    ...
    for (auto& lemma : lemmas) {
      ...
      if (lemma.lemma.compare(0, d->lemma_id_len(lemma.lemma.c_str(), lemma.lemma.size()), tokens[1], 0, lemmaid_len) != 0) continue;
      matches[LEMMA_ID] = 1;
    ...
    if (matches[COMPLETE]) {
      ...
      printf("%s\t%s\t%s\n", tokens[0].c_str(), matching_lemmas[RAWLEMMA][0].lemma.c_str(), matching_lemmas[RAWLEMMA][0].tag.c_str());
               ...
               lemmas[0].lemma.compare(0, d->lemma_id_len(lemmas[0].lemma.c_str(), lemmas[0].lemma.size()), tokens[1], 0, lemmaid_len) == 0) {
